...
bool swordsman::attack(player &p)
{
	...
	{
		HPtemp=(int)((hit*AP/(p.DP+ p.intelligence))*AP*5/(rand()%4+10));		// opponent's HP decrement calculated based their AP/DP, and uncertain chance
	...
	// Normal attack
	HPtemp=(int)((hit*AP/(p.DP+ p.intelligence))*AP*10/(rand()%4+10));
...
bool swordsman::specialatt(player &p)
{
	...
	else
	{
		...
		//double hit=1;			
		//srand(time(NULL));		
		HPtemp=(int)(AP*2+20*AP/(p.DP));		// not related to opponent's DP
...
void swordsman::printSkill(int x) {
	cout << skill[x];
}