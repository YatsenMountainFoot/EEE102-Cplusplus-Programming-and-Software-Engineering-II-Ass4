...
bool archer::attack(player &p)
{
	...
	{
		HPtemp = (int)((hit*AP / (p.DP + p.speed))*AP * 5 / (rand() % 4 + 10));		// opponent's HP decrement calculated based their AP/DP, and uncertain chance
	...
	// Normal attack
	HPtemp = (int)((hit*AP / (p.DP + p.speed))*AP * 15 / (rand() % 4 + 10));
